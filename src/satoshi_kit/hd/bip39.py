...
assert len(WORDLIST) == 2048  # noqa: S101
...
class Mnemonic:
    ...
    def generate(self, strength: int = 128) -> str:
        """Generate a new mnemonic with given strength in bits."""
        if strength not in (128, 160, 192, 224, 256):
            raise ValueError(f"Invalid strength: {strength}. Allowed: 128, 160, 192, 224, 256")
    ...
    def to_entropy(self, words: str | list[str]) -> bytearray:
        ...
        if len(words) not in (12, 15, 18, 21, 24):
            raise ValueError(f"Word count must be 12, 15, 18, 21, or 24 (got {len(words)})")

        concat_len_bits = len(words) * 11
        concat_bits = [False] * concat_len_bits

        for word_index, word in enumerate(words):
            ndx = bisect.bisect_left(self.wordlist, word)
            if ndx >= len(self.wordlist) or self.wordlist[ndx] != word:
                raise LookupError(f'Unable to find "{word}" in word list.')
            for ii in range(11):
                concat_bits[(word_index * 11) + ii] = (ndx & (1 << (10 - ii))) != 0
    ...
    def to_mnemonic(self, data: bytes) -> str:
        """Convert entropy bytes to a mnemonic string."""
        if len(data) not in (16, 20, 24, 28, 32):
            raise ValueError(f"Data length must be 16, 20, 24, 28, or 32 (got {len(data)})")
    ...
    def check(self, mnemonic: str) -> bool:
        ...
        if len(mnemonic_list) not in (12, 15, 18, 21, 24):
            return False
        try:
            idx = [bin(self.wordlist.index(x))[2:].zfill(11) for x in mnemonic_list]
            b = "".join(idx)
        except ValueError:
            return False
        length = len(b)
        ...
        nh = bin(int(hashlib.sha256(nd).hexdigest(), 16))[2:].zfill(256)[: length // 33]
        return h == nh

    @classmethod
    def to_seed(cls, mnemonic: str, passphrase: str = "") -> bytes:
        """Derive a 64-byte seed from mnemonic + optional passphrase."""
        mnemonic_n = cls.normalize_string(mnemonic)
        passphrase_n = cls.normalize_string(passphrase)
        salt = "mnemonic" + passphrase_n
